...
async function sendCustomerEmail(product: ProductKey, c: Payload["customer"]) {
  const fromAddress =
  ...
  const html = `
    ...
    <hr/>
    <p>Customer is being redirected to Stripe Checkout.</p>
  `;
...
export async function POST(req: Request) {
  try {
    const { product, customer } = (await req.json()) as Payload;
    ...
    // Email first (await while debugging; switch to fire-and-forget later)
    await sendCustomerEmail(product, customer);
